...
- Profile scan rules are migrated from Conformity `included` rules with these mappings:
  ...
  - Custom risk level settings (`selectedSeverity`, `customizedRiskLevel`, and snake_case variants) are not mapped. If detected, the migration emits a comment in the `values` block noting the unmapped `customized_risk_level` so you can review and set it explicitly if needed.
  ...
  - WARNING: The verification tooling may normalize `choice-multiple-value-with-tags` to `choice-multiple-value` when the API drops empty `customized_tags`. This can hide a real mismatch if tags are actually set but dropped. Consult the Vision One provider documentation and test your specific rules to confirm how these fields are treated in the API and adjust expectations accordingly. If you rely on customized tags, review the generated HCL and update it explicitly (do not rely on `terraform plan` diffs, since normalization can mask changes).
