...
Only ADMIN/POWER users can create organisation-level and group-level report-configs.
 ...
 - `configuration` - (Required) List:
     ...
     *  `generate_report_type` (String) - (Optional) Valid values are COMPLIANCE-STANDARD and GENERIC. By default, GENERIC type reports are produced. If COMPLIANCE-STANDARD is specified, filter.reportComplianceStandardId has to be:
      ...
      b.) one of the following supported values which can be found from the filter.reportComplianceStandardId parameter of https://automation.trendmicro.com/cloudone/conformity/#tag/Report-Configs/paths/~1report-configs/post
...
- `filter` - (Optional) List:
     ...
     * `compliance_standards` (Array of Strings) - (Optional) An array of supported standard or framework ids. Available values can be found from the filter.complianceStandards parameter of https://automation.trendmicro.com/cloudone/conformity/#tag/Report-Configs/paths/~1report-configs/post
. If none is specified, all standards are set
     ...
     * `report_compliance_standard_id` (String) - (Optional) A single standard or framework id string. Available values can be found from the filter.reportComplianceStandardId parameter of https://automation.trendmicro.com/cloudone/conformity/#tag/Report-Configs/paths/~1report-configs/post
