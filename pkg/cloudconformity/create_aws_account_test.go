...
func TestCreateAwsAccountSuccess(t *testing.T) {
	// prepare the test
	...
	// run the code
	accountID, err := client.CreateAwsAccount(AccountPayload{})
	// check the results
	assert.Nil(t, err)
	assert.Equal(t, accountID, expectedAccountID)
}

func TestCreateAwsAccountFail(t *testing.T) {
	// prepare the test
	client, ts := createHttpTestClient(t, http.StatusForbidden, errResponse)
	defer ts.Close()

	// run the code
	externalID, err := client.CreateAwsAccount(AccountPayload{})
	// check the results
	assert.NotNil(t, err)
