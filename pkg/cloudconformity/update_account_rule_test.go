...
func TestUpdateAccountRuleSettingsSuccess(t *testing.T) {
	// prepare the test
	...
	defer ts.Close()
	// run the code
	accountId, err := client.UpdateAccountRuleSettings("some-id", "some-rule-id", &AccountRuleSettings{})
	// check the results
	assert.Nil(t, err)
	assert.Equal(t, accountId, expectedAccountId)

}
func TestUpdateAccountRuleSettingsFail(t *testing.T) {
	// prepare the test
	client, ts := createHttpTestClient(t, http.StatusForbidden, errResponse)
	defer ts.Close()
	// run the code
	awsAccountId, err := client.UpdateAccountRuleSettings("some-id", "some-rule-id", &AccountRuleSettings{})
	// check the results
	assert.NotNil(t, err)
