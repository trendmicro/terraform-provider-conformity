...
func TestGetCheckSuccess(t *testing.T) {
	// prepare the test
	client, ts := createHttpTestClient(t, http.StatusOK, testGetCheckSuccessResponse)
	defer ts.Close()
	// run the code
	check, err := client.GetCheck("ccc:8d99dfce-dca2-4f13-8699-20631a5c77c9:Resources-001:Resources:global:%2Fsubscriptions%2Fae9124e0-d61c-4d7d-833d-c58e6f9941f8%2Fproviders%2FMicrosoft.Authorization%2FroleDefinitions%2F00482a5a-887f-4fb3-b363-3b7fe8e74483")
	// check the results
	assert.Nil(t, err)
...
func TestGetCheckFail(t *testing.T) {
	// prepare the test
	client, ts := createHttpTestClient(t, http.StatusForbidden, errResponse)
	defer ts.Close()
	// run the code
	_, err := client.GetCheck("ccc:8d99dfce-dca2-4f13-8699-20631a5c77c9:Resources-001:Resources:global:%2Fsubscriptions%2Fae9124e0-d61c-4d7d-833d-c58e6f9941f8%2Fproviders%2FMicrosoft.Authorization%2FroleDefinitions%2F00482a5a-887f-4fb3-b363-3b7fe8e74483")
	// check the results
	assert.NotNil(t, err)
	assert.EqualError(t, err, errResponse)
}

func TestGetCheckInvalidCheckId(t *testing.T) {
	// prepare the test
	client, ts := createHttpTestClient(t, http.StatusForbidden, errResponse)
	defer ts.Close()
	// run the code
	_, err := client.GetCheck("invalid-check-id")
	// check the results
	assert.NotNil(t, err)
	assert.EqualError(t, err, "invalid check-id: 'invalid-check-id'")
}

var testGetCheckSuccessResponse = `
{
  "data": {
    "type": "checks",
